...
void odeFcn_main(double t, int G, double* y, double* dydt, int fibreX, int fibreY, Params* parameters)
{
    cc++;
    //Display("y", y, G);
...
*/
	...
	if( F_vec[Params::N-1]>0)
	{dF[Params::N - 1] = -parameters->gamma*F_vec[Params::N-1]/(F_vec[Params::N-1]+parameters->eta)*F_vec[Params::N-1];}
	else 
	{dF[Params::N - 1] = 0;}
    // Fibre starts at location (fibreX,fibreY) and extends horizontally to the right
    for (int i = 0; i < Params::L; i++)
        dy(grid_radius + fibreX + i, grid_radius + fibreY) += -1*dF[Params::N - 1];
...
void DiffuseSimple(double t0, double tfinal, double* y, int grid_size, int fibreX, int fibreY, Params* parameters)
{
    ...
    for (int t = t0; t < tfinal; t++)
    {
        ...
        odeFcn_main(t, grid_size, y, dy, fibreX, fibreY, parameters);
...
void Diffuse(double t0, double tfinal, double* y0, int grid_size, int fibreX, int fibreY, Params* parameters)
{
    int NN = grid_size * grid_size + Params::N;
    ...
    odeFcn_main(t0, grid_size, y0, f1, fibreX, fibreY, parameters);
    ...
    while (!done)
    {
        ...
        while (true)
        {
            ...
            double t2 = t + h * a2;
            odeFcn_main(t2, grid_size, ynew, f2, fibreX, fibreY, parameters);

            for (int i = 0; i < NN; i++)
                ynew[i] = y[i] + h * (b21 * f1[i] + b22 * f2[i]);
            double t3 = t + h * a3;
            odeFcn_main(t3, grid_size, ynew, f3, fibreX, fibreY, parameters);

            for (int i = 0; i < NN; i++)
                ynew[i] = y[i] + h * (b31 * f1[i] + b32 * f2[i] + b33 * f3[i]);
            double t4 = t + h * a4;
            odeFcn_main(t4, grid_size, ynew, f4, fibreX, fibreY, parameters);

            for (int i = 0; i < NN; i++)
                ynew[i] = y[i] + h * (b41 * f1[i] + b42 * f2[i] + b43 * f3[i] + b44 * f4[i]);
            double t5 = t + h * a5;
            odeFcn_main(t5, grid_size, ynew, f5, fibreX, fibreY, parameters);

            for (int i = 0; i < NN; i++)
                ynew[i] = y[i] + h * (b51 * f1[i] + b52 * f2[i] + b53 * f3[i] + b54 * f4[i] + b55 * f5[i]);
            double t6 = t + h;
            odeFcn_main(t6, grid_size, ynew, f6, fibreX, fibreY, parameters);
            ...
            odeFcn_main(tnew, grid_size, ynew, f7, fibreX, fibreY, parameters);
