...
gamma = 1e-4; % Release rate from fibre
eta = 2;
...
p = clib.Model.SeedAndGrowToStartVolumeM(p0, psc, dmax, gage, page, EC50, gamma, eta, tumour_volume_initial); % simulates the growth of a tumour starting at 1 cell and then stopping when the tumour volume reaches tumour_volume_initial
...
for jj = 1:4 % does 250 different simulations of tumour growth
    ...
    psim = clib.Model.CreateNewParticle(p0, psc, dmax, gage, page, EC50, gamma, eta, p); %sets the initial tumour size for the simulation as the size of the tumour "p", i.e. tumour_volume_initial
